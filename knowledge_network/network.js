...
// Determine data URLs based on environment
...
function initNetwork() {
    try {
        ...
        // Set adaptive Canvas size (smaller for mobile)
...
function fitToViewport() {
    ...
    // Add padding (10% of the network size)
    const padding = Math.max(dx, dy) * 0.1;
    const scale = Math.min(
        canvas.node().width / (dx + padding * 2),
        canvas.node().height / (dy + padding * 2)
    );

    const transform = d3.zoomIdentity
        .translate(canvas.node().width / 2, canvas.node().height / 2)
        .scale(Math.min(scale, 1)) // Don't zoom out beyond 1x
        .translate(-x, -y);
...
function drawCanvas() {
    ...
    links.forEach(d => {
        if (d.source?.x && d.source?.y && d.target?.x && d.target?.y) {
            ...
            ctx.strokeStyle = edgeTypeColors[d.type] || "#FF00FF"; // Magenta for visibility
    ...
    nodes.forEach(d => {
        if (d.x && d.y) {
            ...
            // Add node labels when zoomed in (k > 0.5)
            if (transform.k > 0.5) {
...
// Filter nodes/links by current layer (exact matching)
...
function startSimulation() {
    ...
    // Optimized force parameters for large networks
    simulation = d3.forceSimulation(nodes)
        .force("link", d3.forceLink(links).id(d => d.id).distance(150))
        .force("charge", d3.forceManyBody().strength(-2000)) // Strong repulsion
        .force("center", d3.forceCenter(width / 2, height / 2).strength(1.0)) // Strong center
    ...
    simulation.nodes(nodes).on("tick", () => {
        tickCount++;
        // Throttle redrawing (every 3 ticks)
        if (tickCount % 3 === 0) {
            drawCanvas();
        }
...
function filterNetworkByDomain(domain) {
    ...
    drawCanvas(); // Redraw to show highlights
}
